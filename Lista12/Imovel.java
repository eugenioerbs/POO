...
public class Imovel {
    ...
    public double getArea() {
        if (area <= 0) {
            throw new IllegalArgumentException("Area não pode ser zero nem negativa");
        }
        return area;
    }
    ...
    public Double calcularIptu (Imovel i, double area) { //verificar com o tio chat se esta certo assim, acho que não funcionar por conta do else
        double result = 0;
        
        if (finalidade == null ) {
            throw new IllegalArgumentException("Deve ser informada a finalidade");
        } else {
